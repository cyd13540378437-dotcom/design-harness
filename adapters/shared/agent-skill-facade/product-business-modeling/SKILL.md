...
4. In a target project, read `docs/product/business-modeling/BUSINESS_MODEL_OVERVIEW.md` first when it exists, then load only the detailed source assets needed for the task.
...
The user-facing decision compression must also appear in `docs/product/business-modeling/BUSINESS_MODEL_OVERVIEW.md`, so the user can decide without opening task files.
