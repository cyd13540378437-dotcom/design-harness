...
approved_gates:
  - gate: "prototype-approval"
    ...
    summary: "用户批准在现有 DE-001 视觉基线内做轻量修复，不重新打开整体样式任务。"
...
统一 idea-storm-lab 内容模块中“问题”文本的长度换行表现，避免长问题撑破卡片、影响列表扫描和移动端阅读，同时保持 DE-001 已确认视觉基线。
...
- 列表项高度变化可控，不造成相邻模块遮挡。
- 390px 移动宽度下问题文本仍可读。
- 实现遵守 `VISUAL_DESIGN.md` 的配色、密度和组件语言。
...
沿用 DE-001：创作者持续整理想法的工作台，不要传统后台，也不要营销站。

### 参考图片与解析

- 已读取 [../../VISUAL_DESIGN.md](../../VISUAL_DESIGN.md)。
- 继续遵守 warm creator tool 与 editorial content system 的已确认解析。
- 本任务不新增参考图片。

### 已确认配色

- 沿用 DE-001 已确认配色：暖中性背景、深墨绿主色、泥土色次色、柔和琥珀末级强调色。

### 已确认设计禁区

- 继续避免普通 SaaS 后台感、蓝紫 AI 渐变、玻璃拟态、营销大 Hero、过度圆角和过度留白。

### 视觉原型摘要

- Lightweight 修复，不重新创建完整视觉原型。
- 使用长文本测试数据验证卡片、详情区和移动单列布局。

### 视觉基线文档

- 已读取 `VISUAL_DESIGN.md`。
- 本任务只修复长文本换行，不改变视觉基线，因此无需更新。
...
- 不更新 `VISUAL_DESIGN.md`，因为视觉基线没有变化。
...
- 视觉基线检查：颜色、间距、边框和组件语言未偏离 `VISUAL_DESIGN.md`。
...
- 2026-06-21T09:20:00Z：读取 `VISUAL_DESIGN.md`，确认本任务遵守既有视觉基线。
