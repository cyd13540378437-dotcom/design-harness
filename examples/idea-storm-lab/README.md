...
本示例展示 Codex Design Harness v0.1.0-alpha 要求的三个请求序列。
...
candidates: []
reason: "这是具有独立目标、视觉方向、原型、实现和验收标准的 UI/UX 任务。"
user_question: ""
recommended_action: "创建 DE-001 并进入 Full Mode。"
```

`DE-001` 示例状态已经关闭并封存，便于阅读完整生命周期。
...
user_question: ""
recommended_action: "创建 DE-002，记录 predecessor: DE-001。"
```

`DE-002` 保持 active，并停在 `completion-approval`，用于展示“实现可完成，但最终关闭仍属于用户”的状态。
