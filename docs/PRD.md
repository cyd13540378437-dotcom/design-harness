...
- **版本**：v0.1.1-alpha / reference-library increment
- **文档状态**：可执行草案
- **目标仓库名**：`codex-design-harness`
- **产品显示名**：Design Engineering Harness
- **主要语言**：简体中文；配置字段、状态枚举、文件名使用英文
- **发布日期目标**：完成首个可手动复用、可真实试运行的 GitHub 仓库
- **本版核心变化**：在 v0.1.0-alpha visual-workflow revision 的 Work Item / STATE.md / Gate / Steward / VISUAL_DESIGN.md 基础上，新增 Product Visual Reference Library、Surface Resolution 和 Work Item 级 `REFERENCE_SELECTION.md`。
...
- **`reference-library/`** 是项目长期视觉参考资料层；
- **`REFERENCE_SELECTION.md`** 是单个 Work Item 如何消费参考库的记录；
...
13. 没有结构化产品视觉参考库时，Agent 容易泛泛引用知名产品，或把 mobile app、Web 工作台和响应式 Web 的参考错误混用；
14. 用户不应被要求理解 “Linear 的密度”“Notion 的低噪音” 这类专业参考判断，Agent 应把参考转译成业务语义和取舍。
...
11. 提供面向非 UX 用户的视觉流程：视觉种子、参考图片、参考解析、配色确认、字体字号确认、设计禁区确认、视觉原型；
12. 提供自制视觉参考图片包，供用户没有参考图时选择；
13. 在项目模板中提供 `VISUAL_DESIGN.md`，用于沉淀已确认的项目视觉基线；
14. 示例必须展示配色确认、字体字号确认、设计禁区确认和视觉设计文档的生成或更新；
15. 在项目模板中新增 `docs/design/reference-library/`，作为长期产品视觉参考资料层；
16. 在视觉方向前新增 Surface Resolution，先明确目标终端再检索参考；
17. 为每个状态化 UI/UX Work Item 提供 `REFERENCE_SELECTION.md`，记录本任务实际采用和排除的参考；
18. 让用户批准业务语义方向和取舍，不要求用户理解具体参考产品的专业优势。
...
- 自动下载第三方截图、自动 OCR 或视觉识别截图；
- 在 `reference-library/` 下创建 Work Item 专属 `reference-packs/`；
...
本版不新增 `palette-approval`、`typography-approval` 或 `design-exclusion-approval` 枚举。配色确认、字体字号确认和设计禁区确认是 `visual-direction-approval` 下的视觉子检查点。
...
Typography 是视觉流程中必须在视觉原型前确认的字体字号与排版系统。它不是单个 `font-family`，而是字体族、fallback、字号阶梯、行高、字重、用途映射、响应式调整、授权/加载说明和可读性风险的组合。

对用户展示时必须使用同构 UI 样张，而不是只给字体名或字号表。样张至少覆盖：
...
如果项目已有明确 `VISUAL_DESIGN.md` 或设计系统排版基线，且当前任务不改变它，可以继承并记录为已确认。否则应进入 `typography-selection` 子检查点。

### 7.11 Design Exclusions
...
设计禁区由 Agent 根据产品上下文、视觉种子、参考图片、已选配色和已确认排版系统生成，再交给用户确认。

### 7.12 `VISUAL_DESIGN.md`
...
VISUAL_DESIGN.md
记录项目长期复用的视觉基线：参考图、配色、字体字号、禁区、组件风格、token、示例。
```

`VISUAL_DESIGN.md` 不要求用户一开始填写，而是在视觉方向、原型或生产实现被确认后由 Agent 创建或更新。

### 7.13 Visual Reference Pack
...
### 7.13 Product Visual Reference Library

Product Visual Reference Library 是项目级、长期、可版本化的视觉参考资料层。
...
它保存跨任务复用的产品参考、设计模式、来源、截图素材目录、色卡预留区和 schema。它不保存某个 Work Item 的专属选择结果。
...
原因是 `reference-packs` 会混淆长期资料层和 Work Item 级消费结果。

### 7.14 Surface Target

Surface Target 是本次设计任务要落地的目标终端。
...
找参考前必须先明确 Surface Target。如果终端可从用户请求、项目结构、技术栈或已有状态判断，Agent 记录假设并继续；只有终端不明确且会显著影响参考选择、原型范围或实现方式时，才询问用户。

### 7.15 Reference Selection

Reference Selection 是某个 Work Item 从 Reference Library 中筛选出的任务级参考集合。
...
它属于 Work Item，不属于 Reference Library。`STATE.md` 只记录 Reference Selection 链接、目标终端、采用原则摘要、用户批准方向和关键排除项。
...
### 8.3.1 参考资源是 Agent 的证据，不是用户的专业词汇

用户不需要理解具体产品的设计优势，也不需要在产品名之间做选择。
...
Web、mobile app、responsive web、desktop app 和 tablet 参考不能直接混用。跨终端参考只能用于抽象模式，不得直接用于布局、密度、导航、手势或视觉比例。
...
- 如果视觉种子、参考图、配色、字体字号和设计禁区已经清晰，Agent 应输出 1 个主方案，必要时附 1 个备选；
...
5. 检查 `docs/design/reference-library/` 是否存在；
6. 显式调用 `design_state_steward`；
7. 获得五类结果之一；
8. 若为 `AMBIGUOUS`，请求用户决定并停止。

### 阶段 1：业务理解、Surface Resolution 与视觉种子
...
如果终端可以从用户请求、项目结构、技术栈或已有状态判断，Agent 直接记录假设并继续。只有当终端不明确且会显著影响参考选择、原型范围或实现方式时，才询问用户。
...
### 阶段 2：Reference Library、参考图片收集与解析

Agent 应优先使用具体视觉材料。

如果项目存在 `docs/design/reference-library/`，Agent 必须先按以下顺序检索：
...
该文件至少包含 Work Item、Surface Target、Task Diagnosis、Reference Query、Selected References、Rejected References、Design Principles、User-facing Direction Summary 和 Copyright Boundary。

`visual-direction-approval` Gate 给用户的选项必须是业务语义，而不是产品名。参考产品可以作为说明依据，但用户不需要理解具体参考产品的专业优势。
...
4. 项目 `reference-library/` 中匹配当前终端和任务的产品或模式参考；
5. 本仓库内置的 `visual-reference-packs`；
6. 用户只提供文字时，使用 Visual Seed 生成初始候选。
...
用户确认配色后，必须先记录到 `STATE.md` 的 `## 已批准决定`，并记录候选调色盘对比摘要、用户原始颜色输入、已确认配色、最终配色来源和选择理由，再继续字体字号步骤。
...
配色确认后，Agent 应提出或继承字体字号与排版系统。若项目已有明确 `VISUAL_DESIGN.md` 或设计系统排版基线，且本任务不改变它，可以记录为继承；否则必须给出可比较的字体字号样张。

字体字号样张至少包含：

- 标题、正文、导航、按钮、表单标签、辅助说明、caption；
- 数据/代码或长内容场景；
- 移动端或窄屏表现；
- 字体族和 fallback stack；
- display / title / heading / body / label / caption / code-data 的字号阶梯；
- 行高、字重、字距；
- 授权、加载和 fallback 风险；
- 中文/英文混排、长文本换行、密集信息和按钮文字容纳风险。

#### 子检查点：字体字号确认

在请求用户确认字体字号前，必须先让 `design_state_steward` 更新 `STATE.md`：
...
等待用户：确认候选排版系统，或说明需要调整的字体、字号、密度或阅读感。
```

用户确认字体字号后，必须先记录到 `STATE.md` 的 `## 已批准决定`，并记录候选样张对比摘要、用户原始字体/字号输入、已确认排版系统、最终排版来源和选择理由，再继续设计禁区步骤。

最终排版来源可以是：

```text
existing-baseline / candidate / modified-candidate / custom
```

### 阶段 5：设计禁区

字体字号与排版系统确认后，Agent 应根据以下材料生成设计禁区：
...
视觉原型以已确认的参考图理解、配色、字体字号与排版系统和设计禁区为准。
...
以批准后的状态、配色、字体字号与排版系统、设计禁区和原型为准：
...
- 用户原始 Visual Seed；
- 目标终端和判断证据；
- `REFERENCE_SELECTION.md` 链接；
- 本任务采用的参考原则摘要；
- 明确排除的参考或跨终端风险；
- 参考图片路径或来源摘要；
- 参考图解析的关键结论；
- 已确认配色名称和核心色值；
- 已确认字体族、fallback、字号阶梯、行高、字重和用途映射；
- 已确认设计禁区；
- 原型入口或截图路径；
- 是否已更新 `VISUAL_DESIGN.md`。

不要把完整视觉规范、完整参考分析、产品截图或长表格写进每个 `STATE.md`。
...
│       ├── 11-web-surface-reference-selection.md
│       ├── 12-mobile-surface-reference-selection.md
│       └── 13-reference-selection-belongs-to-work-item.md
│
└── docs/
    ├── PRD.md
    ├── PRD.reference-library-v0.1.1.md
    ├── PRD.v0.1-to-v0.1.1-delta.md
    ├── architecture.md
...
description: 对新产品界面、重要功能页面、跨页面用户流程和显著 UI/UX 改造执行人参与的设计工程工作流。负责识别或恢复 Work Item，执行 Surface Resolution、参考库消费、配色确认、字体字号确认、设计禁区确认、视觉原型、关键交互与完成确认 Gate，并在跨 Codex 对话中通过 STATE.md 恢复状态。
---
```
...
14. 视觉方向前执行 Surface Resolution；
15. 如果项目存在 `docs/design/reference-library/`，必须按终端和任务内容检索；
16. 本任务采用和排除的参考必须写入 Work Item 的 `REFERENCE_SELECTION.md`；
17. 给用户的视觉方向必须用业务语言表达，不要求用户理解具体参考产品的专业优势；
18. 完整视觉设计前必须提出并确认至少 3 套候选调色盘；
19. 完整视觉设计前必须提出并确认设计禁区；
20. 原型必须体现已确认配色、字体字号、设计禁区和目标终端；
21. 实现前必须读取已有 `VISUAL_DESIGN.md` 和本任务 `REFERENCE_SELECTION.md`，如果存在；
22. 视觉 Work Item 完成前必须创建或更新 `VISUAL_DESIGN.md`，除非说明无需更新的理由。
...
interface:
  ...
  short_description: "交互式 UX 探索、Surface Resolution、参考库消费、配色确认、任务状态恢复、原型确认、实现与视觉验收"
  default_prompt: "使用 $design-engineering 推进当前产品设计任务。先解析或恢复 Work Item，再明确目标终端并检索参考库，按适用的 Human Gate 完成参考解析、配色确认、字体字号确认、设计禁区确认、原型、实现、验证和关闭确认。"
...
- 在用户回复后记录批准决定；
- 记录父级 Agent 提供的 Surface Target、Reference Selection 链接、参考原则摘要和关键排除项；
- 记录已确认配色、字体字号和设计禁区；
...
- 检索或挑选 Reference Library 中的参考条目；
- 代替用户通过 Gate；
- 修改 sealed 状态；
- 将完整聊天内容写入状态；
- 将完整参考分析、产品截图或长表格写入 `STATE.md`；
- 在 `reference-library/` 下创建 Work Item 专属文件或目录；
- 在 `AMBIGUOUS` 时自行选择；
...
字体族、fallback、字号阶梯、行高、字重、用途映射、响应式调整、授权/加载说明和可读性风险。
...
- 已创建 `REFERENCE_SELECTION.md`，并记录 responsive-web 目标终端、采用原则和排除项；
- 保持 DE-001 已确认视觉基线；
- 只处理内容模块长文本换行问题；
- 不修改 sealed 的 DE-001。
...
- And：该文件包含已确认配色、字体字号、设计禁区、组件语言和示例；
- And：后续 DE-002 必须读取并遵守该视觉基线。

### 22.11 Web Surface Reference Selection

- Given：目标项目存在 `docs/design/reference-library/`；
- When：用户要求优化 Web 管理后台首页；
- Then：必须先记录 `primary_surface: web-app` 或 `responsive-web`；
- And：优先选择 Web SaaS dashboard、dense productivity workspace、data-card dashboard 等参考；
- And：原生 mobile app 参考只能作为抽象模式补充；
- And：创建或更新 Work Item 级 `REFERENCE_SELECTION.md`；
- And：不得创建 `docs/design/reference-library/reference-packs/`。

### 22.12 Mobile Surface Reference Selection

- Given：目标项目存在 `docs/design/reference-library/`；
- When：用户要求设计原生移动 App 的记录页；
- Then：必须先记录 `primary_surface: mobile-app`；
- And：优先选择移动端记录、习惯反馈、低刺激或快速输入相关参考；
- And：Web 表格、侧边栏和多列 dashboard 只能作为对象关系参考，不得直接落地布局；
- And：用户看到的方向必须是业务语义，而不是产品名选择。

### 22.13 Reference Selection Belongs to Work Item

- Given：存在 `reference-library/`，当前状态解析结果为 `CREATE DE-003`；
- When：Agent 为 DE-003 选择本次任务参考；
- Then：创建 `docs/design/work-items/DE-003-*/REFERENCE_SELECTION.md`；
- And：`STATE.md` 只记录 Reference Selection 链接和摘要；
- And：不得在 `reference-library/` 下创建 `reference-packs/` 或其他 Work Item 专属目录。
...
2. 它解决的问题；
3. 架构图；
4. Thread / Work Item / STATE / Gate / Reference Library / REFERENCE_SELECTION / VISUAL_DESIGN 的区别；
5. 五种状态解析结果；
6. Human-in-the-loop 工作流；
7. 新视觉流程：Visual Seed → Surface Resolution → Reference Library / Reference Images → Palette → Typography → Design Exclusions → Prototype；
8. 为什么不要求用户填写产品人格；
9. 如何使用 Reference Library 和内置视觉参考包；
10. 手动安装步骤；
11. 项目初始化步骤；
...
6. 说明项目模板已经包含 `docs/design/reference-library/`；长期可复用产品参考放在该目录，Work Item 本次选择写入对应任务目录下的 `REFERENCE_SELECTION.md`；

7. 开启新的 Codex 会话验证 Skill、Agent 和项目指令；
...
18. 不创建 `reference-library/reference-packs/`；
19. `STATE.md` 只记录 Reference Selection 摘要和链接，不吞掉完整参考分析；
20. 不自动下载、OCR 或复用第三方截图；
21. 仓库自身的 `AGENTS.md` 要求后续修改保持 PRD、Skill、Agent、模板、视觉参考包和示例一致。
...
- `VISUAL_DESIGN.template.md`；
- `REFERENCE_SELECTION.template.md`；
- 项目模板 `docs/design/reference-library/`；
- 产品索引和设计模式索引的 Markdown/YAML；
- 色卡目录预留结构；
...
- [ ] 明确解释 Reference Library、`REFERENCE_SELECTION.md` 与 `STATE.md` 的区别；
- [ ] 明确解释 Surface Resolution 和跨终端参考限制；
- [ ] 明确解释为什么不要求用户填写产品人格；
- [ ] 视觉流程说明足够非 UX 用户理解。
...
- [ ] 完整视觉设计前必须确认配色；
- [ ] 完整视觉设计前必须确认设计禁区；
- [ ] 视觉方向前必须执行 Surface Resolution；
- [ ] 存在 `reference-library/` 时必须创建或更新 Work Item 级 `REFERENCE_SELECTION.md`；
- [ ] 用户方向选择用业务语言表达，不要求用户理解具体参考产品优势；
- [ ] 不要求产品人格输入；
- [ ] 没有反同质化 Gate。
...
- [ ] 能记录配色、字体字号、设计禁区和视觉文档更新状态；
- [ ] 只记录 Surface Target、Reference Selection 链接和摘要，不挑选参考；
- [ ] 不在 `reference-library/` 下创建任务专属文件；
- [ ] 不代替用户批准视觉子检查点。
...
- [ ] 示例状态与索引一致；
- [ ] successor 关系明确；
- [ ] `STATE.template.md` 包含设计对象与终端、参考资源摘要；
- [ ] `STATE.md` 只记录 Reference Selection 链接和摘要；
- [ ] 视觉输入、参考解析、已确认配色、字体字号和设计禁区可恢复。
...
- [ ] 每个产品条目包含 surface、page type、task type、borrow、avoid 和 source；
- [ ] 每个模式条目包含 suitable surfaces、risky surfaces、avoid 和 QA checks；
- [ ] 色卡目录存在，但不预填具体色卡；
- [ ] `REFERENCE_SELECTION.template.md` 存在于 Skill assets 和项目模板；
...
- [ ] “检查项目上下文”不创建状态；
- [ ] “优化前端样式”创建并封存 DE-001；
- [ ] “优化前端样式”展示参考解析、配色确认、字体字号确认、设计禁区确认和原型确认；
...
- [ ] Reference Library 评测覆盖 Web/mobile 参考不会错误混用；
- [ ] Reference Selection 评测覆盖任务级文件不写入 `reference-library/`；
...
### Milestone 3：视觉流程、参考库与参考包

- 完成 `visual-workflow.md`；
- 完成 `reference-library-consumption.md`；
- 创建项目模板 `reference-library/`；
- 创建产品索引、模式索引、色卡预留目录和 schema；
...
Codex Design Harness v0.1.1 不是一个自动化平台，而是一份能够在真实项目中运行的、可版本控制的产品开发协议：
...
Surface Resolution 是参考检索前的终端判断
REFERENCE_SELECTION.md 是任务级参考消费记录
Visual Seed 是低成本视觉输入
Reference Images 是视觉证据
Palette、Typography 和 Design Exclusions 是可确认中间产物
