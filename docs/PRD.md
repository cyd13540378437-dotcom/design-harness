...
description: 对新产品界面、重要功能页面、跨页面用户流程和显著 UI/UX 改造执行人参与的设计工程工作流。负责识别或恢复 Work Item，执行 Surface Resolution、参考库消费、配色确认、设计禁区确认、视觉原型、关键交互与完成确认 Gate，并在跨 Codex 对话中通过 STATE.md 恢复状态。
---
```
...
interface:
  ...
  default_prompt: "使用 $design-engineering 推进当前产品设计任务。先解析或恢复 Work Item，再明确目标终端并检索参考库，按适用的 Human Gate 完成参考解析、配色确认、设计禁区确认、原型、实现、验证和关闭确认。"
...
7. 新视觉流程：Visual Seed → Surface Resolution → Reference Library / Reference Images → Palette → Design Exclusions → Prototype；
8. 为什么不要求用户填写产品人格；
