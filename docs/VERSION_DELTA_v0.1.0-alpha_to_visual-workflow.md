...
每次配色确认必须至少给出 3 套候选调色盘。Agent 可以标注推荐方案，但不得只给 1 套让用户确认。

用户也可以不选择候选，而是直接输入自己的颜色偏好。该输入必须作为 `palette-selection` 的用户决定记录到 `STATE.md`。
...
每套配色都必须配一个小型 UI 样张，不得只给色值。候选配色必须放在同一个调色盘对比板中，使用同构 UI 样张，并说明主要文字、按钮文字、次级文字、边框和标签的可读性/对比情况。用户确认或自定义后，状态必须记录用户原始颜色输入、候选对比摘要、最终配色来源和选择理由。
