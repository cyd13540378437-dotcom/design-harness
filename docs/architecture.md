...
v0.2.3 更新 runtime-aware Product Business Modeling Core。它不是设计工程 Skill 的子流程，而是并行的 Agent-neutral 产品业务语义层：Core 位于 `core/product-design/business-modeling/`，shared Agent Skill Facade 位于 `adapters/shared/agent-skill-facade/product-business-modeling/`，运行时 adapter 位于 `adapters/<runtime>/`，目标项目资产位于 `docs/product/`。Business Modeling 通过 `BUSINESS_MODEL_OVERVIEW.md` 给产品负责人提供单一默认入口，通过 downstream views 向 Design Engineering 提供业务语义输入，但不直接修改 `docs/design/`。

v0.1.4 更新 standalone Product Business Analysis Core。它同样不是设计工程 Skill 的子流程，也不是业务建模或 Motivation Analysis 的触发层；它是独立、Choice-first 且具备选择闭环边界的商业决策支持能力：Core 位于 `core/product-design/business-analysis/`，运行时 adapter 片段位于 `adapters/<runtime>/product-business-analysis/`，目标项目资产位于 `docs/product/business-analysis/`。v0.1.4 保留 Choice Core、provisional Choice Set、Recommendation Obligation、Choice Delta、Claim-level Assurance、No Dead-End Rule、Decision Closure、`ANALYSIS_WORKSPACE.md` 和输出经济性，并新增 Mixed-turn Interpretation、Choice Loop Closure、Decision Scope Lock、Post-decision Clarification、Artifact Boundary Gate、No Execution Offer 和 State Enum Enforcement。Business Analysis 不生成跨模块 Trigger、downstream views 或共享 Work Item。
...
Assurance 不再用 Case 级全局 `block` 收尾。整体成熟度为 `pass / warn / partial`，关键声明单独标记为 `supported / provisional / conditional / blocked`。缺少资料时执行 No Dead-End Rule：仍比较现实路径并给出当前推荐。用户确认后必须关闭原 Choice Loop、冻结选择快照、写入 `DECISION_NOTES.md`，并将 Case 推进到 `completion-review`；确认后的澄清只能解释原业务决策层的目的、理由、取舍、证据边界和复审条件。

Business Analysis 使用 schema 3 State，并固定枚举：`status=active|paused|completed|cancelled`，`phase=input-understanding|case-routing|choice-set|analysis|recommendation|human-decision|post-decision-clarification|review|completion-review|complete`，`gate=none|decision-confirmation|completion-approval`。不得将业务主题、实验阶段或实施步骤写成自定义 phase / gate。Artifact Boundary Gate 禁止实施 Runbook、手工指南、数据 Schema、样例数据、公式权重、脚本、UI / 技术规格和执行结果；No Execution Offer 禁止主动提出生成、运行、编写、搭建或实现。

本层采用五个运行域：统筹与路由、核心决策分析、复审/风险/决策记忆、材料接收/项目提取/质量保障、Human Decision Control Plane。Passive Trigger、Motivation Analysis 集成和跨模块影响传播不属于当前版本；未来若需要，应作为独立 Integration Layer 设计。
