...
Palette 是视觉流程中必须先于完整视觉设计确认的配色方案。
...
- 输入不清晰或参考图冲突时：输出 2–3 套配色；
- 输入清晰且参考图一致时：可输出 1 套推荐配色 + 1 套保守备选；
- 不得只给色值，必须给 UI 样张。
...
等待用户：选择配色 A / B / C，或说明需要调整的颜色方向。
```

用户确认配色后，必须先记录到 `STATE.md` 的 `## 已批准决定`，再继续设计禁区步骤。
