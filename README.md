...
`product-business-analysis` v0.1.4 is a standalone, choice-first decision-support capability with explicit choice-closure boundaries. Its Core lives under `core/product-design/business-analysis/`, runtime adapter fragments live under `adapters/codex/product-business-analysis/`, `adapters/claude-code/product-business-analysis/`, and `adapters/cursor/product-business-analysis/`, and project templates live under `templates/project/docs/product/business-analysis/`. Start with [docs/CODEX_BUILD_BRIEF.product-business-analysis-core-v0.1.4-complete.md](docs/CODEX_BUILD_BRIEF.product-business-analysis-core-v0.1.4-complete.md) and [docs/PRD.product-business-analysis-v0.1.4.md](docs/PRD.product-business-analysis-v0.1.4.md).
...
Manual Given / When / Then scenarios live in [evals/scenarios](evals/scenarios/) for Design Engineering, [evals/product-business-modeling](evals/product-business-modeling/) for Business Modeling, and [evals/product-business-analysis](evals/product-business-analysis/) for Business Analysis. They cover context checks, create, resume, successor, parallel Work Items, ambiguous binding, close-and-seal, the visual workflow, Reference Library and Color Card Registry behavior, lifecycle memory, business modeling source-of-truth boundaries, passive triggers, Human Decision Control Plane, schema-view limits, UX Context Pack consumption, single overview delivery, canonical BM Work Item files, frozen State Schema, stable references, post-write validation, Business Analysis Decision Case boundaries, Analysis Model Card usage, follow-up continuity, Choice Sets, Recommendation Obligation, Choice Delta, No Dead-End behavior, claim-level Assurance, Decision Closure, Mixed-turn Interpretation, Choice Loop Closure, Decision Scope Lock, post-decision clarification, artifact boundaries, No Execution Offer, State Enum Enforcement, Chinese output, ARCOS confirmation regression, assurance checks, and sealed-state immutability.
...
- Product Business Modeling v0.2.3 is runtime-aware but document-based: no CLI, hook, plugin package, installer, external runtime dependency, automatic database migration, or API generation.
- Product Business Analysis v0.1.4 is standalone and document-based: no Motivation Analysis integration, cross-capability triggers, downstream views, shared Work Item directory, implementation runbook, sample data, script, UI / technical design, execution offer, CLI, hook, plugin package, installer, external runtime dependency, automatic D2/D3 human decision, or solution-design implementation workflow.
