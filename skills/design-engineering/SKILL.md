...
description: 对新产品界面、重要功能页面、跨页面用户流程和显著 UI/UX 改造执行人参与的设计工程工作流。负责识别或恢复 Work Item，执行 Surface Resolution、参考库消费、Visual Seed、参考解析、配色确认、设计禁区确认、视觉原型、实现验证与完成确认，并通过 STATE.md、REFERENCE_SELECTION.md 与 VISUAL_DESIGN.md 跨 Codex 对话恢复状态。
---
...
8. 若结果为 `AMBIGUOUS`，请用户选择并停止。
9. 若结果为 `NO_STATE`，不创建设计状态，直接完成当前请求。
...
按照已批准状态、Surface Target、Reference Selection、配色、设计禁区和原型实现。复用本地组件和设计令牌。不要创建平行设计系统，也不要静默偏离已批准方向，不直接照搬第三方截图或品牌资产。

若出现新的高影响歧义，返回对应 Gate。

### 8. 审查与验证

优先使用真实浏览器按目标终端验证，并对照 `REFERENCE_SELECTION.md` 中的设计原则检查。将证据记录到 `STATE.md`。如果无法完成真实浏览器验证，记录为 blocker 或剩余风险；不得声称视觉 QA 已通过。
